...
const columnWidths = [100, 100, 100, 100]; // Ancho de las columnas (puedes ajustarlo según tu diseño)

// Encabezados de la tabla
doc.fontSize(14).font('Helvetica-Bold').text('Num Arete', startX, startY);
doc.text('Precio', startX + columnWidths[0], startY);
doc.text('Peso', startX + columnWidths[0] + columnWidths[1], startY);
doc.text('Clasificación', startX + columnWidths[0] + columnWidths[1] + columnWidths[2], startY);
...
venta.ganado.forEach((animal) => {
  ...
  doc.text(`$${animal.price}`, startX + columnWidths[0], startY);
  doc.text(`${animal.weight} Kgs`, startX + columnWidths[0] + columnWidths[1], startY);
  doc.text(animal.classification, startX + columnWidths[0] + columnWidths[1] + columnWidths[2], startY);
